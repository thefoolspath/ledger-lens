...
- Plan 0004 documents the accepted future unified API response direction: successful business JSON will use a shared `{ data, meta }` envelope, failures will remain RFC Problem Details with stable error codes and trace/correlation metadata, and English/Thai user-facing text will come from RESX resources. This is planning only; current runtime response shapes and behavior have not changed.
...
Continue confirmed-ledger Milestone 2 work without reusing simulation tables or projections: buys, sells, fees, taxes, dividends, correction/reversal semantics, UI, and reconciliation coverage remain. Plan 0004 remains documentation-only until its response-contract migration is explicitly scheduled. Correct the current machine's invalid fixed-user UUID User Secret before relying on an ordinary persistent `lg run`; ephemeral synthetic command-line configuration remains verified. Keep the 32.178-second warm-start observation visible while collecting enough repeat runs to estimate p95.
