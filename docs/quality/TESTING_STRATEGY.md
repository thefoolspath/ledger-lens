...
- **COMPILED / BLOCKED:** the synthetic distributed scenario now exercises symbol/quote/FX, duplicate draft confirmation, two buys, partial sell, simulation correction, current valuation, and an unchanged confirmed cash/entry projection. Its project builds with zero warnings/errors, but current execution and PostgreSQL migration/isolation evidence are blocked because Docker reports no daemon pipe.
